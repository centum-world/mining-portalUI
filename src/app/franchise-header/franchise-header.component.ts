...
export class FranchiseHeaderComponent implements OnInit {
      ...
      next: (response: any) => {
        if (response) {
          (this.franchiseDetails.fname = response.franchise.fname),
            ...
            (this.franchiseDetails.franchiseId =
              response.franchise.franchiseId),
  ...
  openFranchiseDocumentsDialog() {
    ...
    this.userService.fetchParticularFranchiseDetails(data).subscribe({
      next: (response: any) => {
        if (response) {
          (this.franchiseDocuments.aadharFrontSide =
            response.franchise.adhar_back_side),
            (this.franchiseDocuments.aadharBackSide =
              response.franchise.adhar_back_side),
    ...
    let config: MatDialogConfig = {
      panelClass: "franchiseDocumentsDialogClass",
      data: this.franchiseDocuments,
    };
    const dialogRef = this.dialog.open(
